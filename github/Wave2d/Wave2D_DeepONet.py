...
def main():
    ...
    model = DeepONetMulti(config['model']['branch1_layers'], config['model']['branch2_layers'], config['model']['trunk_layers'], config['model']['act']).to(device)
    print('Number pf model parameters: %s' % (count_params(model)))
    ...
    if not args.test: 
        ...
        error_L2 = np.linalg.norm(s_test - s_pred) / np.linalg.norm(s_test)
        ...
        # Plot on CPU
        u_test, y_test, s_test = generate_plot_data(config)
        u_test, y_test = torch.from_numpy(u_test).float(), torch.from_numpy(y_test).float()
        u_test, y_test = u_test.to(device), y_test.to(device)
        u1_test, u2_test, u3_test = u_test[:, 0:m1], u_test[:, m1:2*m1], u_test[:, 2*m1:]
        s_pred = model(u1_test, u2_test, u3_test, y_test)
        ...
        pltSol(config, y_test, s_pred, s_test, dir, name)
