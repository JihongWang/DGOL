...
def test_data():
    a = 0.01
    u = lambda x, t: np.exp(-a*x**2)*np.cos(t-x)
    u0 = partial(u, t=np.zeros(1))
    ut0 = lambda x: np.exp(-a*x**2)*np.sin(x)
    u_tt = lambda x, t: -np.exp(-a*x**2)*np.cos(t-x)
    u_xx = lambda x, t: np.exp(-a*x**2)*((4*a**2*x**2-2*a-1)*np.cos(t-x)-4*a*x*np.sin(t-x))
...
def train_data(coe):
    A, a, c, k, b, d = coe[:, 0].reshape(-1,1), coe[:, 1].reshape(-1,1), coe[:, 2].reshape(-1,1), coe[:, 3].reshape(-1,1), coe[:, 4].reshape(-1,1), coe[:, 5].reshape(-1,1)
    a = -np.abs(a)
    u = lambda x, t: np.sum(A*np.exp(a*(x+c*t)**2)*np.sin(k*x+b*t+d), axis=0)
    u0 = partial(u, t=np.zeros(1))
    ut0 = lambda x: np.sum(A*np.exp(a*(x)**2)*(2*a*c*x*np.sin(k*x+d)+b*np.cos(k*x+d) ), axis=0)
    u_tt = lambda x, t: A*np.exp(a*(x+c*t)**2)*(np.sin(k*x+b*t+d)*(2*a*c**2*(2*a*(x+c*t)**2+1)-b**2) + np.cos(k*x+b*t+d)*4*a*b*c*(x+c*t))
    u_xx = lambda x, t: A*np.exp(a*(x+c*t)**2)*(np.sin(k*x+b*t+d)*(4*a**2*(c*t+x)**2+2*a-k**2)+ np.cos(k*x+b*t+d)*4*a*k*(x+c*t))
    f = lambda x, t: np.sum(u_tt(x,t) -u_xx(x,t), axis=0)
...
def mixed_train(model,              # model of neural operator
    ...
    loss_it_log, loss_bcs_log, loss_res_log, loss_total_log = [], [], [], []
    error_it_log, error_L2_log, error_L1_log, error_max_log  = [], [], [], []
    ...
    for it in pbar:
        ...
        u1_res, u2_res, u3_res = u_res[:,0:m1], u_res[:,m1:m1+m2], u_res[:,m1+m2:]
        ...
        if (it+1) % 100 == 0:
            loss_it_log.append((it+1)), loss_bcs_log.append(loss_bcs.item())
            loss_res_log.append(loss_res.item()), loss_total_log.append(loss_total.item())
        ...
        if (it+1) % save_it == 0:
            ...
            pltLossPI(torch.tensor(loss_it_log, device = 'cpu'), torch.tensor(loss_bcs_log, device = 'cpu'), \
                    torch.tensor(loss_res_log, device = 'cpu'), torch.tensor(loss_total_log, device = 'cpu'), \
                      save_path, config['train']['save_name'])
            pltError(torch.tensor(error_it_log, device = 'cpu'),
                     torch.tensor(error_L2_log, device = 'cpu'),
                    torch.tensor(error_L1_log, device = 'cpu'),
                    torch.tensor(error_max_log, device = 'cpu'), save_path, config['train']['save_name'])
    ...
    sio.savemat(save_error_name, {'error_it': error_it_log, 'error_L2': error_L2_log, 'error_L1': error_L1_log, 'error_max': error_max_log})
    save_loss_name = '%s/%sLossData.mat' % (save_path, config['train']['save_name'])
    sio.savemat(save_loss_name, {'loss_it': loss_it_log, 'loss_bcs': loss_bcs_log, 'loss_res': loss_res_log, 'loss_total': loss_total_log})



def pltLossPI(loss_it, loss_bcs, loss_res, loss_total, DataFolderPath, name):
 
 
    plt.rcParams.update({'font.size': 15}) 
    fig, ax = plt.subplots(figsize = (6,5))
    plt.plot(loss_it, loss_bcs, lw=2, label='bcs')
    plt.plot(loss_it, loss_res, lw=2, label='res')
    plt.plot(loss_it, loss_total, lw=2, label='total')
...
def generate_one_training_data(config, coe):
    ...
    u = np.hstack((u0(x1).reshape(1,-1), ut0(x2).reshape(1,-1), f_fn(x_f, t_f).reshape(1,-1)))
...
def generate_training_data_PI(config):
    ...
    y_bcs_train = np.zeros((N,P,2))
    s_bcs_train = np.zeros((N,P,1))
    u_res_train = np.zeros((N,Q,m1+m2+m3))
    ...
    for i in range(N):
        ...
        u_res_train[i,:,:], y_res_train[i,:,:], s_res_train[i,:,:] = generate_one_training_data_PI(config, coe)
...
def generate_one_training_data_PI(config, coe):
    ...
    x_f, t_f = np.meshgrid(x_f, t_f)
    x_f, t_f = x_f.reshape(1,-1), t_f.reshape(1,-1)
    u = np.hstack((u0(x1), ut0(x1), f_fn(x_f, t_f)))
...
def generate_one_test_data(config):
    ...
    s_test = u(XX.reshape(1,-1),TT.reshape(1,-1)).flatten()
