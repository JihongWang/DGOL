...
def main():
    ...
    u0_test, ut0_test, f_test = u0_fun_test(x_u).flatten(), ut0_fun_test(x_u).flatten(), f_fun_test(x_f,t_f).flatten()
    ...
    while index < Num:
        ...
        coe = np.zeros((order_x, 6))
        coe[:,0], coe[:,1], coe[:,2], coe[:,3], coe[:,4], coe[:,5] = np.random.normal(1, 0.5, (order_x,)), np.random.normal(0.05, 0.1, (order_x,)), \
                np.random.normal(-1, 0.5, (order_x,)), np.random.normal(0, 0.5, (order_x,)), np.random.normal(0, 0.5, (order_x,)), np.random.normal(1, 1, (order_x,))

 
        u0_fun_train, ut0_fun_train, f_fun_train, u_fun_train = train_data(coe)
        u0_train, ut0_train, f_train = u0_fun_train(x_u).flatten(), ut0_fun_train(x_u).flatten(), f_fun_train(x_f,t_f).flatten()
        ...
        u0_max, ut0_max, f_max = np.max(np.abs(u0_train)), np.max(np.abs(ut0_train)), np.max(np.abs(f_train))
 

        if u0_err <= tol['u0'] and  ut0_err <= tol['ut0'] and f_err <= tol['f'] and u0_max >= 0.5:
