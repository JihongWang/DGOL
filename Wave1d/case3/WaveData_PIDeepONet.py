...
def main():
    ...
    if not args.test: 
        ...
        u_bcs_train, y_bcs_train, s_bcs_train, u_res_train, y_res_train, s_res_train = generate_training_data_PI(config)
        ...
        u_res_train, y_res_train, s_res_train = torch.from_numpy(u_res_train).float(), torch.from_numpy(y_res_train).float(), torch.from_numpy(s_res_train).float()
        end = time.perf_counter()
        print(end-start)
        ...
        train_loader_res = DataLoader(TensorDataset(u_res_train, y_res_train, s_res_train), batch_size=config['train']['batch_size'], shuffle=True)
