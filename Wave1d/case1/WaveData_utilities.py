...
def test_data():
    ...
    u = lambda x, t: np.exp(-x**2)*np.cos(t-x)
    u0 = partial(u, t=np.zeros(1))
    ut0 = lambda x: np.exp(-x**2)*np.sin(x)
    f = lambda x, t: -np.exp(-x**2)*np.cos(t-x)+2*x*np.exp(-x**2)*(np.sin(t-x)-2*x*np.cos(t-x))+np.exp(-x**2)*(3*np.cos(t-x)+2*x*np.sin(t-x))
...
def train_data(type_x, type_t, order_x, coe_x, coe_t):
    ...
    if type_x == 'hermite':
        a, b = coe_x[0], coe_x[1]
        ux = lambda x: hefunm(order_x-1, a*x+b)  # [1, Nx]
        ux_xx = lambda x: a**2*hefunm_diff2(order_x-1, a*x+b)  # [1, Nx]
       
    if type_t == 'exp':
        A, mean, std = coe_t[:,0].reshape(-1,1), coe_t[:,1].reshape(-1,1), coe_t[:,2].reshape(-1,1)
        ut = lambda t: A*np.exp(-(t-mean)**2/std**2)
        ut_tt = lambda t: A*np.exp(-(t-mean)**2/std**2) *(4*(t-mean)**2/std**4-2/std**2)
        ut0 = lambda x: np.sum(A*2*mean/std**2*np.exp(-mean**2/std**2)* ux(x), axis=0)
    
    if type_t == 'fourier':
        A, k1, k2 = coe_t[:,0].reshape(-1,1), coe_t[:,1].reshape(-1,1), coe_t[:,2].reshape(-1,1)
        ut = lambda t: A* np.sin(k1*t+k2)
        ut_tt = lambda t: -A*k1**2*np.sin(k1*t+k2)
        ut0 = lambda x: np.sum(A*k1*np.cos(k2)* ux(x), axis=0)


    u = lambda x, t: np.sum(ux(x) * ut(t), axis = 0)

    u0 = partial(u, t=np.zeros(1))
    f = lambda x,t: np.sum(ux(x)*ut_tt(t), axis = 0)- np.sum(ut(t)*ux_xx(x), axis =0)
...
def WaveSolver(config, u0, ut0, f):
    ...
    for i in range(2, Nt):
        Uh[i, 1:-1] = dt**2/dx**2*np.matmul(A, Uh[i-1,1:-1])+ 2*Uh[i-1,1:-1] - Uh[i-2,1:-1] + dt**2*f(x_f, t[i-1])
...
def generate_one_training_data(config, coe_x, coe_t):
    ...
    u0, ut0, f_fn, U = train_data(type_x, type_t, order_x, coe_x, coe_t)
...
def generate_one_test_data(config):
    ...
    x, t, U = WaveSolver(config, u0, ut0, f)
...
def generate_one_test_PI_data(config):
    ...
    u = np.hstack((u0(x1), ut0(x1), f(x_f, t_f).flatten()))


    x_r = np.random.uniform(xmin, xmax, (Q, 1))
    t_r = np.random.uniform(tmin, tmax, (Q, 1))
    u_res = np.tile(u, (Q, 1)) 
    y_res = np.hstack([x_r, t_r])  # sample points for computing PDE residual, can be different form
    s_res = f(x_r.T, t_r.T).reshape(Q, 1)

    return u_res, y_res, s_res 


def eq_residual(model, u1_res, u2_res, u3_res, y_res, s):

    y_res.requires_grad = True
    out_res = model(u1_res, u2_res, u3_res, y_res)
    ...
    out_x_tx = torch.autograd.grad(outputs=out_x, inputs=y_res, grad_outputs = torch.ones_like(out_x), 
                        retain_graph=True, create_graph=True, only_inputs=True, allow_unused=True)[0]
    out_xx = out_x_tx[:,0]
    out_t_tx = torch.autograd.grad(outputs=out_t, inputs=y_res, grad_outputs = torch.ones_like(out_t), 
                        retain_graph=True, create_graph=True, only_inputs=True, allow_unused=True)[0]
    out_tt = out_t_tx[:,1]

    res = out_tt - out_xx - torch.squeeze(s)
