...
def main():
    ...
    for i in range(N):
        ...
        u0_train[i,:], f_train[i,:], u_train = u0_fun_train(x_u), f_fun_train(x_f,t_f), u_fun_train(x_f,t_f)
        f_err = np.max(np.abs(f_test - f_train[i,:]))

        WaveSolver_train(config, u0_fun_train, ut0_fun_train, f_fun_train, u_fun_train)
